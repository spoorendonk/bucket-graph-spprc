...
TEST_CASE("ResourcePack with StandardResource") {
    ...
    auto pack = make_resource_pack(StandardResource(consumption, lb, ub, 0, 2, 2));
...
TEST_CASE("Mixed ResourcePack extend_to_vertex composes correctly") {
    ...
    auto ng = make_ng5();
    auto pack = make_resource_pack(StandardResource(consumption, lb, ub, 0, 4, 10), std::move(ng));
...
TEST_CASE(
    "NgPathResource: extend_to_vertex v not in own ng-set sets only self "
    "bit") {
...
TEST_CASE("NgPathResource: self-loop blocked when v is in own ng-set") {
...
TEST_CASE("NgPathResource: self-loop allowed when v not in own ng-set") {
...
TEST_CASE("NgPathResource: backward self-loop blocked when v is in own ng-set") {
...
TEST_CASE(
    "NgPathResource: backward extend, to not in N(from) — self-bit "
    "lost") {
    auto ng = make_ng5();
...
TEST_CASE(
    "NgPathResource: asymmetric neighborhoods — from not in N(to) but "
    "to in N(from)") {
...
TEST_CASE("NgPathResource: self-loop remap preserves other neighbor bits") {
...
TEST_CASE("NgPathResource: domination with v not in own ng-set") {
    ...
    CHECK(ng.domination_cost(Direction::Forward, 1, self_only, self_plus_v0) == 0.0);
    // More bits cannot dominate fewer
    CHECK(ng.domination_cost(Direction::Forward, 1, self_plus_v0, self_only) == INF);
    // Equal → dominates
    CHECK(ng.domination_cost(Direction::Forward, 1, self_only, self_only) == 0.0);
}

TEST_CASE(
    "NgPathResource: concatenation trivially passes when fw state is "
    "empty after remap") {
...
static ProblemView make_cycle_pv(int* from, int* to, double* cost, double* time_d, double* tw_lb,
                                 double* tw_ub, const double** arc_res, const double** v_lb,
                                 const double** v_ub) {
    from[0] = 0;
    to[0] = 1;
    cost[0] = -5.0;
...
TEST_CASE("Solver: ng-path blocks cycles, mono and bidir agree") {
    ...
    const double* v_lb[1];
    const double* v_ub[1];
    auto pv = make_cycle_pv(from, to, cost, time_d, tw_lb, tw_ub, arc_res, v_lb, v_ub);
