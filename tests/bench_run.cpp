...
///
/// Usage: ./build/bench_run
/// [spprclib|rcspp|roberti|bidir|stages|eliminate|validate|verify|all]
...
void run_spprclib() {
  ...
  for (auto& r : results) {
    ...
    if (ok)
      ...
      ++fail;
    printf("%-25s %12.0f %12.0f %8.1f %6d %5s\n", r.name.c_str(), r.best_cost,
           r.expected, r.time_ms, r.n_paths, ok ? "OK" : "FAIL");
  }
  printf("\nSPPRCLIB: %d/%d passed\n", pass, pass + fail);
}
...
void run_rcspp_dir(const std::string& dir, const std::string& label) {
  ...
  double total_ms = 0;
  for (auto& r : results) {
    printf("%-15s %12.3f %8.1f %6d\n", r.name.c_str(), r.best_cost, r.time_ms,
           r.n_paths);
...
void run_bidir_rcspp() {
  ...
  printf("%-15s %12s %8s %12s %8s %6s %5s\n", "Instance", "Mono Cost",
         "Mono ms", "Bidir Cost", "Bidir ms", "Paths", "Match");
  printf("%s\n", std::string(75, '-').c_str());
  ...
  for (auto& entry : fs::directory_iterator(dir)) {
    ...
    // Mono
    auto t0 = std::chrono::high_resolution_clock::now();
    NgPathResource ng_mono(pv.n_vertices, pv.n_arcs, pv.arc_from, pv.arc_to,
                           inst.ng_neighbors);
    ...
    NgPathResource ng_bidir(pv.n_vertices, pv.n_arcs, pv.arc_from, pv.arc_to,
                            inst.ng_neighbors);
    Solver<NgPack> bidir(pv, make_resource_pack(std::move(ng_bidir)),
                         {.bucket_steps = {20.0, 50.0},
                          .bidirectional = true,
                          .tolerance = 0.0});
    bidir.build();
  ...
  for (auto& r : rows) {
    ...
    printf("%-15s %12.3f %8.1f %12.3f %8.1f %3d/%3d %5s\n", r.name.c_str(),
           r.mono_cost, r.mono_ms, r.bidir_cost, r.bidir_ms, r.mono_paths,
           r.bidir_paths, r.match ? "OK" : "FAIL");
  }
...
void run_bidir_spprclib() {
  ...
  printf("%-25s %12s %12s %12s %8s %8s %5s\n", "Instance", "Expected", "Mono",
         "Bidir", "Mono ms", "Bi ms", "Match");
  printf("%s\n", std::string(90, '-').c_str());
  ...
  for (auto& entry : fs::directory_iterator(dir)) {
    ...
    rows.push_back(
        {name, expected, mono_best, bidir_best, mono_ms, bidir_ms, match});
  }
  ...
  for (auto& r : rows) {
    ...
    printf("%-25s %12.0f %12.0f %12.0f %8.1f %8.1f %5s\n", r.name.c_str(),
           r.expected, r.mono_cost, r.bidir_cost, r.mono_ms, r.bidir_ms,
           r.match ? "OK" : "FAIL");
  }
  printf("\nBidir SPPRCLIB: %d/%d mono==bidir, %d/%d <= optimal\n", matched,
         (int)rows.size(), vs_opt_ok, (int)rows.size());
}
...
void run_bidir_roberti() {
  ...
  int with_opt = 0;
  for (auto& r : rows)
    if (r.expected < -1e-9) ++with_opt;
  printf("\nBidir Roberti: %d/%d mono==bidir, %d/%d <= optimal\n", matched,
         (int)rows.size(), vs_opt_ok, with_opt);
}
...
void run_stages() {
  ...
  for (auto& entry : fs::directory_iterator(dir)) {
    ...
    auto run_with_stage = [&](Stage stage) -> std::pair<double, double> {
      ...
      Solver<EmptyPack> solver(
          pv, EmptyPack{}, {.bucket_steps = {20.0, 50.0}, .tolerance = 0.0});
      solver.build();
  ...
  for (auto& r : rows) {
    if (std::abs(r.h1_cost - r.exact_cost) < 1.0 ||
        r.h1_cost <= r.exact_cost + 1e-6)
      ++h1_match;
    if (std::abs(r.h2_cost - r.exact_cost) < 1.0 ||
        r.h2_cost <= r.exact_cost + 1e-6)
      ++h2_match;
    printf("%-15s %12.3f %8.1f %12.3f %8.1f %12.3f %8.1f\n", r.name.c_str(),
           r.h1_cost, r.h1_ms, r.h2_cost, r.h2_ms, r.exact_cost, r.exact_ms);
  }
  printf("\nStages: H1 matched exact %d/%d, H2 matched exact %d/%d\n", h1_match,
         (int)rows.size(), h2_match, (int)rows.size());
}
...
void run_elimination() {
  ...
  for (auto& entry : fs::directory_iterator(dir)) {
    ...
    double best_before =
        paths_before.empty() ? 1e18 : paths_before[0].reduced_cost;
    ...
    double best_after =
        paths_after.empty() ? 1e18 : paths_after[0].reduced_cost;
    ...
    printf("%-15s %12.3f %12.3f %8d %8d %5s\n", name.c_str(), best_before,
           best_after, arcs_before - arcs_after, jumps, match ? "OK" : "FAIL");
...
void run_bucket_fixing() {
  ...
  for (auto& entry : fs::directory_iterator(dir)) {
    ...
    printf("%-15s %12.3f %12.3f %7d %7d %6.1f%% %5s\n", name.c_str(),
           best_before, best_after, n_total, bg.n_fixed_buckets(), pct,
           match ? "OK" : "FAIL");
...
void run_path_validation() {
  ...
  auto validate = [&](const ProblemView& pv,
                      const std::vector<typename Solver<EmptyPack>::Path>&
                          paths,
    ...
    for (const auto& p : paths) {
      ...
      for (size_t i = 0; i < p.arcs.size() && ok; ++i) {
        ...
        if (a < 0 || a >= pv.n_arcs) {
          ...
          printf(
              "  %s %s: arc %d doesn't connect v%d->v%d (expected v%d->v%d)\n",
              mode, name, a, p.vertices[i], p.vertices[i + 1], pv.arc_from[a],
              pv.arc_to[a]);
          ok = false;
        }
      }
...
static void write_csv_row(std::ofstream& out, const std::string& name,
                          double mono, double bidir, double optimal,
                          bool pass) {
  ...
  // ── SPPRCLIB (with ng-path) ──
  {
    ...
    if (fs::exists(dir)) {
      ...
      for (auto& name : names) {
        ...
        auto mono_paths = mono.solve();
        double mono_best =
            mono_paths.empty() ? 1e18 : mono_paths[0].reduced_cost;
        ...
        printf("  %-25s  mono=%12.3f  bidir=%12.3f  %s", name.c_str(),
               mono_best, bidir_best, pass ? "PASS" : "FAIL");
