...
int main() {
    ...
    // Build complete graph (source→customers, customers→customers, customers→sink)
    ...
    const double* arc_res[] = {time_consumption.data(), demand_consumption.data()};
    ...
    for (std::size_t i = 0; i < std::min(paths.size(), std::size_t{5}); ++i) {
        std::printf("  Path %zu: rc=%.1f, orig_cost=%.1f, vertices=",
                    i, paths[i].reduced_cost, paths[i].original_cost);
        for (int v : paths[i].vertices) std::printf("%d ", v);
        std::printf("\n");
    }
