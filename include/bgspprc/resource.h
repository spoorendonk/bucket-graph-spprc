...
concept Resource =
    requires(const R& r, Direction dir, Symmetry sym, typename R::State s,
      ...
      {
        r.extend_along_arc(dir, s, arc_id)
      } -> std::same_as<std::pair<typename R::State, double>>;
      {
        r.extend_to_vertex(dir, s, vertex)
      } -> std::same_as<std::pair<typename R::State, double>>;
...
struct ResourcePack {
  ...
  bool symmetric() const {
    return std::apply([](const auto&... rs) { return (rs.symmetric() && ...); },
                      resources);
  }

  StatesTuple init_states(Direction dir) const {
    return std::apply(
        [dir](const auto&... rs) { return StatesTuple{rs.init_state(dir)...}; },
        resources);
  }
