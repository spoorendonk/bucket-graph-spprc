...
struct StandardResource {
    ...
    StandardResource(const double* consumption_, const double* lb_, const double* ub_, int source_,
                     int sink_, int n_arcs_)
    ...
    std::pair<State, double> extend_to_vertex(Direction dir, State q, int vertex) const {
        if (dir == Direction::Forward) {
