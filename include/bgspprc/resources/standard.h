...
/// Domination cost: 0 (pure feasibility resource — handled by main resource comparison).
/// Concatenation cost: 0 (feasibility checked via main resource bounds).
///
/// This resource is typically used as one of the 1-2 "main resources" that define
/// the bucket grid. Main resource extension is handled directly in the bucket graph
/// labeling loop (not via the resource pack) for performance.
///
/// When used as a non-main resource in the pack, it provides additional resource
/// constraints (e.g., a third resource beyond the 2 main ones).
struct StandardResource {
    ...
    StandardResource(const double* consumption_, const double* lb_, const double* ub_,
                     int source_, int sink_, int n_arcs_)
