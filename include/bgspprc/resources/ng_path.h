...
///   init_state()       → 0 (BucketGraph calls extend_to_vertex to mark source/sink)
///   extend_along_arc(fw, s, a) → extendAlongArc: check forbidden, remap bits, do NOT mark
///   extend_to_vertex(fw, s, v) → extendToVertex: set v's self bit
///
...
/// j's self bit set). Since fw doesn't have j's self bit, j isn't double-counted.
struct NgPathResource {
    ...
    /// Initialize state: empty (BucketGraph calls extend_to_vertex to mark source/sink).
    State init_state(Direction /*dir*/) const {
        return 0ULL;
    }

    /// extendAlongArc: transform bit vector from source ordering to target ordering.
    /// Does NOT mark the destination vertex — that's done by extend_to_vertex.
    ...
    /// Concatenation: across-arc (i,j) produces fw state at j via extend_along_arc only
    /// (no self bit for j), while bw state at j has j's self bit set.
    /// Any common bit means a vertex visited in both directions → cycle.
