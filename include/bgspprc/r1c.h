...
///
/// Specialized for 3-Subset Row Cuts (|C|=3, p_v=1/2), which are the most common.
/// State per cut: 1 bit (s ∈ {0, 1/2}), packed 64 per uint64_t word.
///
...
private:
    void build_masks() {
        ...
        vertex_toggle_mask_.assign(n_vertices_, std::vector<uint64_t>(n_words_, 0ULL));
